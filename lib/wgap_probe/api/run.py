...
def main(**kwargs):
    ...
    if 'gid_min' in config.filter:
        bpf_text = bpf_text.replace('GID_FILTER','gid < %i' % config.filter.gid_min )
    else:
        bpf_text = bpf_text.replace('GID_FILTER','0')

    if 'uid_min' in config.filter:
        bpf_text = bpf_text.replace('UID_FILTER','uid < %i' % config.filter.uid_min )
    else:
    ...
    while 1:
        ...
        for k, v in reversed(sorted(counts.items(),
            ...
            name = k.name
            if name in ['access.log','processes.log','error.log','other_vhosts_access.log']:
                continue
            k.user = get_username(k.uid)
            print("%s %s" % (k.user, k.name))
            if not k.user in ['planet-work']:
                 continue
            print("%-6d %-16s %-6d %-6d %-7d %-7d %1s %s" % (k.pid, k.user,
